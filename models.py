    ...
    def __init__(self, dataset, emb_dim, **kwargs):
        ...
        if self.non_linearity == "relu":
            self.layers = nn.Sequential(
                nn.Linear(self.emb_dim * self.max_arity + 1, self.hidden_size),
                nn.ReLU(),
...
class LinearModel(BaseClass):
    ...
    def forward(self, r_idx, e1_idx, e2_idx, e3_idx, e4_idx, e5_idx, e6_idx):
        ...
        r = r.reshape(r.shape[0], r.shape[1], self.w, self.b)
        entity_embs = entity_embs.reshape(entity_embs.shape[0], entity_embs.shape[1], self.w, self.b)
        output = torch.sum(r * entity_embs , (2, 1)) + r_bias
        output = self.hidden_drop(output)
...
class RealEv2(BaseClass):
    def __init__(self, dataset, emb_dim, **kwargs):
        super(RealEv2, self).__init__()
        self.emb_dim = emb_dim
        self.max_arity = 6
        self.hidden_drop_rate = kwargs["hidden_drop"]
        self.hidden_drop = torch.nn.Dropout(self.hidden_drop_rate)
        ...
        self.fc1 = torch.nn.Linear(self.b, self.b//2)
        self.fc2 = torch.nn.Linear(self.b//2, 1)
    ...
    def forward(self, r_idx, e1_idx, e2_idx, e3_idx, e4_idx, e5_idx, e6_idx):
        ...
        results = self.apply_non_linearity(self.non_linearity, torch.sum(r * entity_embs , (2, 1)) + r_bias)
        results = self.fc1(results)
        results = self.apply_non_linearity(self.non_linearity, results)
        results = self.fc2(results)
        # results = self.apply_non_linearity('tanh', results)

        return results.reshape(r_idx.shape[0])
...
class ERMLP(BaseClass):
    def __init__(self, dataset, emb_dim, **kwargs):
        ...
        self.non_linearity = kwargs["non_linearity"]
        # self.hidden_size = kwargs["filt_w"]
        self.fc1 = torch.nn.Linear(emb_dim * 7, 200)
...
class RealEv4(BaseClass):
    def __init__(self, dataset, emb_dim, **kwargs):
        super(RealEv4, self).__init__()
        self.emb_dim = emb_dim
        self.max_arity = 6
        self.hidden_drop_rate = kwargs["hidden_drop"]
        self.hidden_drop = torch.nn.Dropout(self.hidden_drop_rate)
        ...
        self.R1 = torch.nn.Embedding(dataset.num_rel(), emb_dim * self.max_arity, padding_idx=0)
        self.R1_bias = torch.nn.Embedding(dataset.num_rel(), self.b, padding_idx=0)
        self.R2 = torch.nn.Embedding(dataset.num_rel(), emb_dim * self.max_arity, padding_idx=0)
        self.R2_bias = torch.nn.Embedding(dataset.num_rel(), self.b, padding_idx=0)
        self.R3 = torch.nn.Embedding(dataset.num_rel(), emb_dim * self.max_arity, padding_idx=0)
        self.R3_bias = torch.nn.Embedding(dataset.num_rel(), self.b, padding_idx=0)
        self.R4 = torch.nn.Embedding(dataset.num_rel(), emb_dim * self.max_arity, padding_idx=0)
        self.R4_bias = torch.nn.Embedding(dataset.num_rel(), self.b, padding_idx=0)
        self.R5 = torch.nn.Embedding(dataset.num_rel(), emb_dim * self.max_arity, padding_idx=0)
        self.R5_bias = torch.nn.Embedding(dataset.num_rel(), self.b, padding_idx=0)
        self.R6 = torch.nn.Embedding(dataset.num_rel(), emb_dim * self.max_arity, padding_idx=0)
        self.R6_bias = torch.nn.Embedding(dataset.num_rel(), self.b, padding_idx=0)
    ...
    def init(self):
        ...
        xavier_normal_(self.R1_bias.weight.data)
        xavier_normal_(self.R2.weight.data)
        xavier_normal_(self.R2_bias.weight.data)
        xavier_normal_(self.R3.weight.data)
        xavier_normal_(self.R3_bias.weight.data)
        xavier_normal_(self.R4.weight.data)
        xavier_normal_(self.R4_bias.weight.data)
        xavier_normal_(self.R5.weight.data)
        xavier_normal_(self.R5_bias.weight.data)
        xavier_normal_(self.R6.weight.data)
        xavier_normal_(self.R6_bias.weight.data)
    ...
    def forward(self, r_idx, e1_idx, e2_idx, e3_idx, e4_idx, e5_idx, e6_idx):
        ...
        r1 = self.R1(r_idx).reshape(-1, self.max_arity, self.emb_dim)
        r1_bias = self.R1_bias(r_idx)
        r2 = self.R2(r_idx).reshape(-1, self.max_arity, self.emb_dim)
        r2_bias = self.R2_bias(r_idx)
        r3 = self.R3(r_idx).reshape(-1, self.max_arity, self.emb_dim)
        r3_bias = self.R3_bias(r_idx)
        r4 = self.R4(r_idx).reshape(-1, self.max_arity, self.emb_dim)
        r4_bias = self.R4_bias(r_idx)
        r5 = self.R5(r_idx).reshape(-1, self.max_arity, self.emb_dim)
        r5_bias = self.R5_bias(r_idx)
        r6 = self.R6(r_idx).reshape(-1, self.max_arity, self.emb_dim)
        r6_bias = self.R6_bias(r_idx)
        ...
        r1 = r1.reshape(r1.shape[0], r1.shape[1], self.w, self.b)
        r2 = r2.reshape(r2.shape[0], r2.shape[1], self.w, self.b)
        r3 = r3.reshape(r3.shape[0], r3.shape[1], self.w, self.b)
        r4 = r4.reshape(r4.shape[0], r4.shape[1], self.w, self.b)
        r5 = r5.reshape(r5.shape[0], r5.shape[1], self.w, self.b)
        r6 = r6.reshape(r6.shape[0], r6.shape[1], self.w, self.b)

        entity_embs = entity_embs.reshape(entity_embs.shape[0], entity_embs.shape[1], self.w, self.b)
        
        results = torch.cat(
                (torch.sum(self.apply_non_linearity(self.non_linearity, torch.sum(r1 * entity_embs , (2, 1)) + r1_bias), dim=1).unsqueeze(1) \
                , torch.sum(self.apply_non_linearity(self.non_linearity, torch.sum(r2 * entity_embs , (2, 1)) + r2_bias), dim=1).unsqueeze(1) \
                , torch.sum(self.apply_non_linearity(self.non_linearity, torch.sum(r3 * entity_embs , (2, 1)) + r3_bias), dim=1).unsqueeze(1) \
                , torch.sum(self.apply_non_linearity(self.non_linearity, torch.sum(r4 * entity_embs , (2, 1)) + r4_bias), dim=1).unsqueeze(1) \
                , torch.sum(self.apply_non_linearity(self.non_linearity, torch.sum(r5 * entity_embs , (2, 1)) + r5_bias), dim=1).unsqueeze(1) \
                , torch.sum(self.apply_non_linearity(self.non_linearity, torch.sum(r6 * entity_embs , (2, 1)) + r6_bias), dim=1).unsqueeze(1))
                , 1)
        
        output = self.fc1(results)
        output = self.hidden_drop1(output)
        output = self.apply_non_linearity('sigmoid', output)
        output = self.fc2(output).reshape(r_idx.shape[0])
